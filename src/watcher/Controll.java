...
public class Controll {
	...
	private JSONmanager jm;		//DeviceControll controlls all Device related funktions and acts as a file interface
	...
	private int devicetype;				//the Typ ot the twa device
	
	//Optionsfile Location
	private String optionFile = System.getProperty("user.dir") + "/" + "Options.xml";
	private String[] tagNames = {"printTime","cycleTime", "maxCycle", "apiToken", "userToken", "sftpHost", "sftpUser", "sftpPw", "sshUser", "sshPW"};
	private String[] options;
	private boolean optionsUse;
	
	/***
	 * Constructor Controll
	 */
	public Controll(Boolean work){
		System.out.println("Starting a background service instance..");
		//creating objekcts
		...
		if(work) {
			System.out.println("Service instance is now running as main..");
	...
	private void loadOptions() {
		    ...
		    if(new File(this.optionFile).exists()) { 	//check if options file exists
		    	if((this.optionsUse = this.xml.checkUse()) == true) { //check if user wants to use the Option.xml
		    	
			    	this.options = this.xml.readOptions();		//call the xmlManager to read the options from file
			    ...
			    this.pm.errorPrint("!- Benutzeroptionen nicht gefunden");
			    this.pm.errorPrint(" - Lade Default Optionen");
	...
	private void showOptions() {
		if(this.optionsUse) {
		this.pm.print(" - Folgende Optionen wurden geladen:");
		for(int i = 0; i < this.options.length; i++) {
			System.out.println("  >> Option: " + this.tagNames[i] + " = " + this.options[i]);
		}
		this.pm.gap();
		this.pm.print(" - Die Optionsdatei finden Sie in:");
		this.pm.print(" -> " + System.getProperty("user.dir") + "\\Options.xml");
		} else {
    		this.pm.print(" - Benutzeroptionen sollen nicht geladen werden");
    		this.pm.print(" - Lade Default Optionen");
		}
	...
	private void work() {
		...
		while(true) {
			...
			ArrayList<Device> devices = this.jm.read();				//fetching all saved Devices
			...
			for(Device device: devices) { 
				...
				this.snmp = new SNMPControll(this.twaName, this.twaIP, this.lastfingerprint, this.fingerprintOID, this);
					...
					case 3: this.confman.checkDeafult(this.twaName, this.twaIP, this.devicetype); 		//check for a safed default or save as new default
							status = 5; 												//everithing was OK, end of work
							break; 
					
					//check the configuration, OK status = 5 - !OK status = 6
					case 4: status = this.confman.configcheck(this.twaName, this.twaIP, this.devicetype);//analyse the Config file, OK returns 5 - ! returns 6
							break;
	...
	/***
	 * public method toErrorPrint
	 * @param pString
	 * Desc:
	 * 	Allowes the workflow to parse a Errorprint
	 */
	public void toErrorPrint(String pString) {
		this.pm.errorPrint(pString);
	}//eom toErrorPrint
	...
	/**
	 * public method getConfigManager
	 * @return ConfigManager
	 * Desc:
	 * 	returns the ConfigManager for the GUI to use
	 */
	public ConfigManager getConfigManager() {
		return this.confman;
	}//eom getConfigManager
}//eoc
