...
public class SNMPControll {
	...
	public SNMPControll(String pname, String pip, String plastfp, OID pfingOID, Controll pcontroll) {
		...
		try {
			...
			this.controll.toErrorPrint("!--- FEHLER:SNMP-MANAGER: " + e);
		}
	...
	public int fingerprintCheck(){
		...
		try {
			...
			this.controll.toErrorPrint("!--- FEHLER:SNMP-MANAGER:FINGERPRINT: " + e);
		}
	...
	public int synCheck(){
		...
		if(!flag.equals("Null")) {		//check if not Null (Bat-fail-save)
	    ...
	    case 2:	status = 4;
	    		...
	    		break;
	    case 3: status = 4;
	    		this.controll.toErrorPrint("!---> WARNUNG! Synflag ist auf hoechster Problemstufe!");
	    		this.controll.toPush(5, "none", this.name, this.ip);
	    		break;
	    default:status = 3;
	    		this.controll.toErrorPrint("!---> Synflag wurde fehlerhaft empfangen");
	    }//eo switch, synflag
